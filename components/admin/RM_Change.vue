<template>
  <div class="section">
    <h3 class="component-header">Change RM from ethVault</h3>
    <div style="display: flex; flex-direction: column;">
      <v-text-field
          label="RM from account"
          variant="solo"
          v-model="fromAccount"
          hint="FromAccount Ratio: "
          persistent-hint
          class="input"
      ></v-text-field>
      <v-text-field
          ...
          variant="solo"
          v-model="toAccount"
          hint="ToAccount Ratio: "
          persistent-hint
          class="input"
      ></v-text-field>
      <v-text-field
          label="Ratio to Change"
          variant="solo"
          v-model="ratioAmount"
          class="input"
      ></v-text-field>
...
const confirm = async() => {
  ...
  if(walletAddress && web3Manager && fromAccount.value!= '' && toAccount.value != '' && ratioAmount.value!=''){
    web3Manager?.changeRiskManager(fromAccount.value, toAccount.value, ratioAmount.value, walletAddress.value).then((res) => {
      alert(res);
    });
  }
};
